...
function PANEL:PostInit()
    ...
    viewer.ViewerFOV = 75
    viewer:SetFOV(viewer.ViewerFOV)
    viewer:SetLookAng(Angle(11, 180, 0))
    viewer:SetCamPos(Vector(100, 0, 55))
    viewer:SetDirectionalLight(BOX_RIGHT, Color(0, 255, 0))
    viewer:SetDirectionalLight(BOX_LEFT, Color(0, 0, 255))
    viewer:SetDirectionalLight(BOX_FRONT, Color(0, 0,0))
    viewer:SetDirectionalLight(BOX_BACK, Color(255, 0, 0))
    viewer:SetDirectionalLight(BOX_TOP, Color(255, 0, 0))
    viewer:SetDirectionalLight(BOX_BOTTOM, Color(255, 0, 0))
    viewer:Dock(FILL)
    viewer:SetAmbientLight(Color(0, 0, 0, 255))

    function viewer:OnMouseWheeled(delta)
        self.ViewerFOV = self.ViewerFOV - delta * 5
    end
    local offsets = {
        ["All"] = 1,
        ["Head"] = 1.15,
    ...
    function viewer:Think()
        ...
        --self:SetFOV(self.ViewerFOV)
        --self:SetCamPos(Vector(100, 0, 55))
        
        local VW    = self:GetWide()
        local VX, _ = self:GetPos()
        local CX, _ = input.GetCursorPos()
        
        local RX = ((CX - VW/2) - VX) / VW
        local DistFromCenter = math.abs(RX - 0.5) * 2
        local Brightness = math.Clamp(300 - DistFromCenter * 300, 0, 255)
        self:SetDirectionalLight(BOX_FRONT, Color(Brightness, Brightness, Brightness))
    ...
    backViewButton:SetText("Развернуть")
