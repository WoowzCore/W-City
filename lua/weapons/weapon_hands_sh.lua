	...
	if SERVER then
		...
		elseif IsValid(tr.Entity) and tr.Entity:IsPlayer() then
			...
			if Dist < self.ReachDistance then
				...
				if owner.organism.superfighter or owner.PlayerClassName == "sc_infiltrator" or (clawClasses[owner.PlayerClassName]) or owner:IsBerserk() then
...
function SWEP:Think()
	...
	if owner:KeyDown(IN_SPEED) or owner:KeyDown(IN_WALK) then
		HoldType = "normal"
	end
